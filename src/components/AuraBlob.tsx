...
export default function AuraBlob({ mood }: AuraBlobProps) {
  ...
  useFrame((state, delta) => {
    ...
    if (materialRef.current) {
      ...
      const targetDistortion = 0.3 + (mood * 0.08) + scroll.offset * 0.2
      const targetSpeed = 1 + mood * 0.5 + scroll.offset * 3
